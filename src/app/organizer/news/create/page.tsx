...
const infoSchema = z.object({
  ...
  type: z.string(),
  imageUrl: z.string().optional().superRefine((value, context) => {
    const type = context.path.includes('type') ? context.path[context.path.indexOf('type') + 1] : undefined;
    if (type === 'news' && !value) {
      context.addIssue({
...
const CreateNewsPage = () => {
  ...
  async function onSubmit(data: FormData) {
    setIsLoading(true)
    try {
      const response = await fetch('/api/organizer/create-info', {
        method: 'POST',
        headers: {
